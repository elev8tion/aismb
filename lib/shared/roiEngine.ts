/**
 * Shared ROI Calculation Engine
 *
 * Single source of truth for all ROI math. Pure function — no React,
 * no framework imports. Used by:
 *   - components/ROICalculator/useROICalculation.ts (React hook wrapper)
 *   - lib/voiceAgent/roiCalculator.ts (server-side wrapper)
 *   - app/api/leads/roi/route.ts (API route)
 */
