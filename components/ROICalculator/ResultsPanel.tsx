...
export default function ResultsPanel({ results, tier, onTierChange }: Props) {
  ...
  return (
    <div className="glass p-4 md:p-6 lg:p-8">
      ...
      >
        <div className="text-xs text-[#0EA5E9] font-semibold mb-1">{rc.results.annualBenefit}</div>
        <div className="text-3xl font-bold text-white">${results.annualBenefit.toLocaleString()}</div>
      </div>
      ...
      >
        <div className="flex items-center justify-between mb-1">
          <span className="text-sm text-[#22C55E] font-semibold">{rc.results.yourRoi}</span>
        ...
        <div className="text-xs text-white/70">
          {rc.results.paysForItself}{results.paybackWeeks} {rc.results.continuesGenerating}
        </div>
      </div>
